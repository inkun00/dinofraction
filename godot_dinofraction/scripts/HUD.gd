...
func _build_dashboard_report_payload() -> Dictionary:
	...
	return {
		...
		"worksheetCorrectByType": UserProfile.correct_by_type.duplicate(true) if UserProfile else {},
		"worksheetWrongByType": UserProfile.wrong_by_type.duplicate(true) if UserProfile else {},
		"worksheetTotalGames": UserProfile.total_games if UserProfile else 0,
